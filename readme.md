...
Var - имеет глобальную или локальную область видимости
let и const имеет блочную область видимости( то есть может использоваться только в блоке кода в котором обьявлена )
2. Почему объявлять переменную через var считается плохим тоном?
Потому то она становится глобальной, что потом может привести к проблемам с памятью.
## Задание
